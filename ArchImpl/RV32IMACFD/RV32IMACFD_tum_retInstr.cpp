...
partInit.code() += "if (((RV32IMACFD*)cpu)->PRIV < 3) {\n";
...
partInit.code() += "*((RV32IMACFD*)cpu)->CSR[768] = s;\n";
...
// -----------------------------------------------------------------------------
partInit.code() += "cpu->nextPc = " + std::to_string(ic.current_address_ + 4U) + "U;\n";
partInit.code() += "if (((RV32IMACFD*)cpu)->PRIV < ((get_field(*((RV32IMACFD*)cpu)->CSR[768], 4194304)) ? (3) : (1))) {\n";
partInit.code() += "cpu->exception = 0; raise(cpu, system, plugin_pointers, 0U, 2);\n";
partInit.code() += "goto instr_exit_" + std::to_string(ic.current_address_) + ";\n";
partInit.code() += "}\n";
partInit.code() += "cpu->nextPc = *((RV32IMACFD*)cpu)->CSR[321];\n";
partInit.code() += "etiss_uint32 s = *((RV32IMACFD*)cpu)->CSR[256];\n";
