...
etiss::int32 RV64IMACFDArch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
    ...
    std::function<void()> disableItr = [cpu]() {
        if (likely((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MIE))
        {
            // Push MIE, SIE, UIE to MPIE, SPIE, UPIE
            etiss_uint32 irq_enable = ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MIE) |
                                      ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_UIE) |
                                      ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_SIE);
            (*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) = (irq_enable << 4) | ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & 0xffffff00);
        }
    };
    ...
    std::function<etiss_uint32(etiss_uint32, etiss_uint32)> handle = [cpu, cause](etiss_uint32 causeCode,
        ...
        case 0x0:
            ...
            if (*((RV64IMACFD *)cpu)->CSR[CSR_MEDELEG] & (1 << (causeCode & 0x1f)))
            {
                // Pop MPIE to MIE
                etiss::log(etiss::VERBOSE, "Exception is delegated to supervisor mode");
                (*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) ^=
                    (((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MPIE) >> 4) ^ ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MIE);
                *((RV64IMACFD *)cpu)->CSR[CSR_SCAUSE] = causeCode;
                // Redo the instruction encoutered exception after handling
                *((RV64IMACFD *)cpu)->CSR[CSR_SEPC] = static_cast<etiss_uint32>(cpu->instructionPointer - 4);
                *((RV64IMACFD *)cpu)->CSR[CSR_SSTATUS] ^= (*((RV64IMACFD *)cpu)->CSR[3088] << 8) ^ (*((RV64IMACFD *)cpu)->CSR[CSR_SSTATUS & MSTATUS_SPP]);
                *((RV64IMACFD *)cpu)->CSR[3088] = PRV_S;
                cpu->instructionPointer = *((RV64IMACFD *)cpu)->CSR[CSR_STVEC] & ~0x3;
            }
            else
            {
                *((RV64IMACFD *)cpu)->CSR[CSR_MCAUSE] = causeCode;
                // Redo the instruction encoutered exception after handling
                *((RV64IMACFD *)cpu)->CSR[CSR_MEPC] = static_cast<etiss_uint32>(cpu->instructionPointer - 4);
                (*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) ^=
                    (*((RV64IMACFD *)cpu)->CSR[3088] << 11) ^ ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MPP);
                *((RV64IMACFD *)cpu)->CSR[3088] = PRV_M;
                ...
                cpu->instructionPointer = *((RV64IMACFD *)cpu)->CSR[CSR_MTVEC] & ~0x3;
            }
            break;
        ...
        case 0x80000000:
            // Check exception delegation
            if (*((RV64IMACFD *)cpu)->CSR[CSR_MIDELEG] & (1 << (causeCode & 0x1f)))
            {
                // Pop MPIE to MIE
                ...
                *((RV64IMACFD *)cpu)->CSR[CSR_SEPC] = static_cast<etiss_uint32>(cpu->instructionPointer);
                *((RV64IMACFD *)cpu)->CSR[CSR_SSTATUS] ^= (*((RV64IMACFD *)cpu)->CSR[3088] << 8) ^ (*((RV64IMACFD *)cpu)->CSR[CSR_SSTATUS] & MSTATUS_SPP);
                *((RV64IMACFD *)cpu)->CSR[3088] = PRV_S;
                if (*((RV64IMACFD *)cpu)->CSR[CSR_STVEC] & 0x1)
                    cpu->instructionPointer = (*((RV64IMACFD *)cpu)->CSR[CSR_STVEC] & ~0x3) + causeCode * 4;
                else
                    cpu->instructionPointer = *((RV64IMACFD *)cpu)->CSR[CSR_STVEC] & ~0x3;
            }
            else
            {
                *((RV64IMACFD *)cpu)->CSR[CSR_MCAUSE] = causeCode;
                // Return to instruction next interrupted one
                *((RV64IMACFD *)cpu)->CSR[CSR_MEPC] = static_cast<etiss_uint32>(cpu->instructionPointer);
                (*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) ^=
                    (*((RV64IMACFD *)cpu)->CSR[3088] << 11) ^ ((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MPP);
                *((RV64IMACFD *)cpu)->CSR[3088] = PRV_M;
                // Customized handler address other than specified in RISC-V ISA manual
                ...
                if (*((RV64IMACFD *)cpu)->CSR[CSR_MTVEC] & 0x1)
                    cpu->instructionPointer = (*((RV64IMACFD *)cpu)->CSR[CSR_MTVEC] & ~0x3) + causeCode * 4;
                else
                    cpu->instructionPointer = *((RV64IMACFD *)cpu)->CSR[CSR_MTVEC] & ~0x3;
            }
            break;
    ...
    case etiss::RETURNCODE::INTERRUPT:
        if (!((*((RV64IMACFD *)cpu)->CSR[CSR_MSTATUS]) & MSTATUS_MIE))
        {
            std::stringstream msg;
        ...
        {
            etiss_uint32 mip_tmp = (*(((RV64IMACFD *)cpu))->CSR[CSR_MIP]);
            if (0 == mip_tmp)
            {
                handledCause = etiss::RETURNCODE::NOERROR;
                break;
            ...
            if (!((*(((RV64IMACFD *)cpu))->CSR[CSR_MIE]) & (1 << irqLine)))
            {
                std::stringstream msg;
                handledCause = etiss::RETURNCODE::NOERROR;
    ...
    case etiss::RETURNCODE::ILLEGALINSTRUCTION:
    {
        ...
        *((RV64IMACFD *)cpu)->CSR[CSR_MTVAL] = static_cast<etiss_uint32>(cpu->instructionPointer);
    ...
    case etiss::RETURNCODE::SYSCALL:
        ...
        disableItr();
        switch (*((RV64IMACFD *)cpu)->CSR[3088])
        {
        case PRV_U:
